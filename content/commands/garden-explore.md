...
| [GARDEN Ecosystem](/glossary/garden/) | Core | Primary interface to the 116-repository knowledge base |
| [/garden-extract](/commands/garden-extract/) | Workflow | Exploration findings feed into extraction operations |
| [/garden-search](/commands/garden-search/) | Complementary | Text search complements structured exploration |
| [/garden-sync](/commands/garden-sync/) | Prerequisite | Sync ensures repositories are current before exploration |
| [/gardener](/commands/gardener/) | Parent | Gardener provides top-level GARDEN management |
| [KuzuDB](/glossary/kuzudb/) | Storage | Knowledge graph stored in KuzuDB for relationship queries |
| [Telemetry](/glossary/telemetry/) | Metrics | Exploration events tracked for usage analysis |
...
**Sync before exploring.** Run [/garden-sync](/commands/garden-sync/) before deep exploration to ensure that repository contents are current. Stale submodules may cause the explorer to miss recently added patterns or files.

**Combine exploration with extraction.** After identifying a relevant pattern or component through exploration, use [/garden-extract](/commands/garden-extract/) to bring the discovered knowledge into the active platform. Exploration without extraction leaves value on the table.
...
| `{:error, :index_stale}` | GARDEN index is older than 24 hours | Run [/garden-sync](/commands/garden-sync/) to update the index |
...
The command supports [NABLA](/glossary/nabla-infinity/) signal plurality by surfacing multiple pattern implementations across different repositories and technology stacks, enabling operators to compare approaches and select the most appropriate solution based on diverse evidence.
