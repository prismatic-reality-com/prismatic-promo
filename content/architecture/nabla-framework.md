...
|---|-------|-------------|-----------|
| 1 | **[Signal Plurality](/glossary/signal-plurality/)** | HARD -- minimum 2 independent signals | Single-source beliefs are indistinguishable from hallucinations |
| 2 | **[Contradiction Preservation](/glossary/contradiction-preservation/)** | HARD -- preserve both sides, never discard | Contradictions are information; resolving them prematurely destroys data |
...
| 4 | **[Time Decay](/glossary/time-decay/)** | HARD -- mandatory timestamps on all beliefs | Stale intelligence is dangerous intelligence |
...
| 7 | **[Provenance Mandatory](/glossary/provenance-mandatory/)** | HARD -- all beliefs must be traceable to origin | Untraceable conclusions cannot be audited, updated, or invalidated |
...
- **Provenance Mandatory** ensures the platform maintains a complete [audit trail](/glossary/audit-trail/), critical for [compliance frameworks](/capabilities/aiad-compliance/) like [NIS2](/glossary/nis2/) and [ZKB](/glossary/zkb/).

## Trinity Gate -- Three-Layer Validation

The [Trinity Gate](/capabilities/trinity-gate/) is the epistemic checkpoint that every belief must pass before it can influence agent decisions or be propagated to downstream consumers. The gate implements three independent validation layers, each grounded in a different mathematical discipline. ALL three must pass -- there is no partial credit.
...
The computational cost of three-gate validation is non-trivial. Gate 1 (structural) runs in O(V + E) time using topological sort cycle detection. Gate 2 (logical) is worst-case exponential but bounded by [belief graph](/glossary/belief-graph/) size limits (maximum 10,000 nodes per validation batch). Gate 3 (formal) delegates to [Lean4](/glossary/lean4/) for proof checking, which adds latency but provides mathematical certainty.
...
The [belief graph](/glossary/belief-graph/) is the data structure that tracks the provenance and dependency relationships between all beliefs in the system. It is implemented as a directed acyclic graph (DAG) where nodes represent beliefs and edges represent supporting relationships.
...
The belief graph integrates with the platform's [ETS storage layer](/glossary/ets/) for in-memory access during real-time operations and with [PostgreSQL](/architecture/postgresql-kuzudb/) for persistent storage. Graph traversal queries that span the entire belief network (such as finding all beliefs that transitively depend on a particular signal source) leverage [KuzuDB](/glossary/kuzudb/) for efficient recursive path queries.

## Epistemic Violation Handling

Violations are classified into four severity levels, each with distinct handling semantics. The violation handler is integrated with the platform's [telemetry system](/architecture/telemetry/) to ensure that all violations are recorded, aggregated, and available for trend analysis.
...
The [formal verification](/glossary/formal-verification/) gate (Gate 3) is intentionally the bottleneck. It provides the strongest guarantees but at the highest cost. For time-critical operations at the exploratory [confidence threshold](/glossary/confidence-threshold/) (tau = 0.60), Gate 3 can be configured as RECOMMENDED rather than MANDATORY, allowing the system to proceed with structural and logical validation alone. Critical decisions (tau = 0.95) always require all three gates.
...
Every [agent](/glossary/agent/) in the platform must respect NABLA's axioms and pass through the Trinity Gate for decisions above the exploratory threshold. The integration pattern is standardized through the agent base module.
...
This integration ensures that the [epistemic pipeline](/glossary/epistemic-pipeline/) is consistently applied across all 400+ agents, from the [safety system](/apps/prismatic-safety/) to the [Perimeter EASM scanner](/apps/prismatic-perimeter/). The framework's enforcement is not optional -- it is compiled into the agent [supervision tree](/glossary/supervision-tree/) and enforced at the [OTP](/glossary/otp/) process level through the platform's [supervision architecture](/architecture/supervision-trees/).

## Future Directions

The NABLA framework continues to evolve. Current research areas include integration with probabilistic programming languages (specifically, extending the Lean4 bridge to support probabilistic proofs), real-time belief graph visualization through [Phoenix LiveView](/architecture/phoenix-liveview/), and cross-agent belief propagation protocols that maintain provenance across the platform's [PubSub infrastructure](/architecture/pubsub/). The goal is a platform where every conclusion, no matter how deeply nested in the agent hierarchy, can be traced back to its originating signals with full confidence accounting at every step.
