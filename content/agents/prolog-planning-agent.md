...
Every plan generated by the prolog-planning-agent undergoes formal verification before delivery, implementing the [NO DOUBTS](@/glossary/no-doubts.md) doctrine in planning operations.
...
| Agent | Relationship |
|-------|-------------|
| [prolog-constraint-agent](@/agents/prolog-constraint-agent.md) | Constraint-based resource allocation within generated plans |
| [prolog-reasoning-agent](@/agents/prolog-reasoning-agent.md) | Logical inference for precondition and effect evaluation |
| [supreme-coordinator](@/agents/supreme-coordinator.md) | High-level objective decomposition into planning problems |
| [pvm-adaptive-scheduler](@/agents/pvm-adaptive-scheduler.md) | Temporal scheduling of planned actions on PVM execution resources |

## Integration Architecture

| Component | Relationship |
|-----------|-------------|
| [Prismatic Agents](@/glossary/prismatic-agents.md) | Runtime execution and lifecycle management of planned actions |
| Prismatic Telemetry | Plan execution monitoring and [metrics](@/glossary/metrics.md) collection |
| [AIAD](@/glossary/aiad.md) [Registry](@/glossary/registry-otp.md) | Agent capability lookup for task-to-agent assignment |
| [SEADF](@/glossary/seadf.md) Pipeline | Evolution planning integration for autonomous improvement cycles |
| [Mycelial Network](@/glossary/mycelial-network.md) | Distributed plan execution coordination across agent processes |

## Enforcement

Plan generation and execution are governed by the [NO MERCY](@/glossary/no-mercy.md) doctrine -- incomplete plans, plans with unsatisfied preconditions, or plans violating safety invariants are rejected without exception. The [Trinity Gate](@/glossary/trinity-gate.md) validates critical plans through structural consistency (plan graph is a valid DAG with no circular dependencies), logical consistency (all preconditions satisfied and effects correctly propagated), and formal necessity (safety invariants proven to hold at every plan step). Plans for production-affecting operations require L3+ authorization and pass through the complete verification pipeline before execution authorization is granted.
