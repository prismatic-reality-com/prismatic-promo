...
The Prismatic Platform comprises over 90 applications organized as an Elixir [umbrella application](/glossary/umbrella-application/), running on the [BEAM](/glossary/beam/) virtual machine with [OTP](/glossary/otp/) supervision trees providing fault tolerance and process isolation. The System Architecture Specialist governs the structural decisions that shape this complex system, from inter-application dependency management to data flow architecture and service boundary definition. It operates under the [AIAD](/glossary/aiad/) standard and enforces the [No Mercy, No Doubts](/glossary/no-doubts/) doctrine, requiring that every architectural decision is backed by evidence and thorough analysis.
...
The [OTP](/glossary/otp/) design principles provide a platform-specific architectural framework that the agent enforces. The "let it crash" philosophy, [supervision trees](/glossary/supervision-tree/), and process-based isolation create an architectural paradigm where fault tolerance is a structural property rather than an afterthought. The agent ensures that all architectural decisions leverage these OTP capabilities rather than reimplementing equivalent mechanisms in less robust ways.
...
The System Architecture Specialist is implemented as an [OTP](/glossary/otp/) [GenServer](/glossary/genserver/) process within the Prismatic architecture subsystem, maintaining a persistent model of the platform's architectural state.
...
The "OTP First" principle ensures that the platform fully leverages the BEAM virtual machine's concurrency model. The agent reviews architectural proposals to verify that stateful components are modeled as processes with appropriate supervision, that message passing is used for inter-process communication, and that [ETS](/glossary/ets/) tables are used for shared read-heavy state rather than introducing external state management systems.
...
**Fault Tolerance Assessment** verifies that the architecture provides appropriate resilience through [supervision trees](/glossary/supervision-tree/), circuit breaker patterns, graceful degradation, and recovery mechanisms. The agent ensures that failure in one component cannot cascade to unrelated system areas.
...
|--------|-----------------|-------------------|
| [Prismatic Agents](/glossary/prismatic-agents/) | Agent lifecycle management | OTP supervision |
| [Prismatic Telemetry](/glossary/telemetry/) | Architecture health metrics | Continuous monitoring |
| [AIAD Registry](/glossary/registry-otp/) | Agent specification repository | Read/Write |
| [Phoenix](/glossary/phoenix/) Framework | Web architecture governance | Configuration oversight |
| [Ecto](/glossary/ecto/) | Data architecture management | Schema review |
| [GARDEN](/glossary/garden/) | Architectural pattern library | Pattern retrieval |
| [Trinity Gate](/glossary/trinity-gate/) | Architectural decision verification | Mandatory review |
...
The core stack of Elixir/OTP, [Phoenix](/glossary/phoenix/), [Ecto](/glossary/ecto/), and [LiveView](/glossary/liveview/) provides the foundation for all web-facing components. PostgreSQL serves as the primary relational database, with [ETS](/glossary/ets/) providing in-memory state management. KuzuDB handles graph data requirements, Meilisearch provides full-text search capabilities, and Redis supports caching and pub/sub messaging.
...
The System Architecture Specialist collaborates with the [technical-assessor](/agents/technical-assessor/) for technology evaluation support and the [tech-debt-analyst](/agents/tech-debt-analyst/) for identifying architectural debt. The [systematic-verifier](/agents/systematic-verifier/) provides verification capabilities for architectural invariants. The [ui-flowbite-specialist](/agents/ui-flowbite-specialist/) contributes frontend architecture expertise within the design system domain.
