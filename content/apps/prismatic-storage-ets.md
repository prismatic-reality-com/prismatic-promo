...
[Prismatic Storage](@/glossary/prismatic-storage.md) [ETS](@/glossary/ets.md) implements the platform's storage adapter [protocol](@/glossary/protocol.md) using Erlang Term Storage (ETS). It provides microsecond-latency data access for hot data, caches, session state, and high-throughput workloads that demand the lowest possible read and write latency across the platform.

Because ETS tables are native to the [BEAM](@/glossary/beam.md) virtual machine, data is stored as raw Erlang terms with zero serialization overhead. This makes ETS the fastest storage option in the Prismatic ecosystem -- reads complete in single-digit microseconds regardless of value complexity, and concurrent read access scales linearly with available CPU cores. The adapter manages table lifecycle through [OTP](@/glossary/otp.md) supervision, monitors memory consumption with configurable thresholds, and provides optional disk persistence for crash recovery.

Prismatic Storage ETS serves as the L1 (first-level) cache in the platform's multi-level caching hierarchy, with [Redis](@/apps/prismatic-storage-redis.md) functioning as L2 and [PostgreSQL](@/apps/prismatic-storage-ecto.md) as the durable persistence layer.
...
### Management
- Automatic table lifecycle managed under OTP [supervision tree](@/glossary/supervision-tree.md)s
...
- Named table [registry](@/glossary/registry-otp.md) for discoverable cross-module access
...
The ETS adapter is consumed by nearly every application in the umbrella. [Prismatic HAWKEYE](@/apps/prismatic-hawkeye.md) stores visitor session profiles in ETS for sub-microsecond [risk score](@/glossary/risk-score.md) retrieval. [Prismatic Auth](@/apps/prismatic-auth.md) maintains active sessions and rate limiter state in ETS tables. The [Prismatic API](@/apps/prismatic-api.md) endpoint registry is ETS-backed for lock-free concurrent reads during request dispatch. [Prismatic Mycelial Nx](@/apps/prismatic-mycelial-nx.md) caches [inference](@/glossary/inference.md) results in ETS to avoid redundant GPU computations.
...
- [Prismatic Storage Redis](@/apps/prismatic-storage-redis.md) -- L2 distributed cache layer
...
- [Adapter Pattern Specialist](@/agents/adapter-pattern-specialist.md) -- Ensures ETS adapter conforms to the PrismaticStorageCore protocol contract
- [Architecture Review Specialist](@/agents/architecture-review-specialist.md) -- Reviews ETS table topology and memory management strategies
- [Elixir Architect](@/agents/elixir-architect.md) -- Validates OTP supervision patterns for ETS table ownership

## Related Capabilities

- [Cross-Domain Flexibility](@/capabilities/cross-domain-flexibility.md) -- ETS adapter serves as the universal L1 cache across all platform domains
- [Quality Gates](@/capabilities/quality-gates.md) -- Contract tests verify adapter protocol compliance before deployment
- [Telemetry Integration](@/capabilities/telemetry-integration.md) -- Storage operation metrics emitted for performance monitoring
