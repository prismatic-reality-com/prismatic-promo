...
The system operates on a pull-based scheduling model where monitoring profiles define which sources to query, at what frequency, and which change types warrant notification. This design allows analysts to configure fine-grained tracking for high-priority targets while applying lighter monitoring to the broader entity population, ensuring efficient allocation of [OSINT](@/glossary/osint.md) query budgets and API rate limits.

Tracking state is persisted in [PostgreSQL](@/apps/prismatic-storage-ecto.md) for durability and historical analysis, while active tracking schedules and recent snapshots are held in [ETS](@/apps/prismatic-storage-ets.md) for low-latency access by the scheduling engine.
...
- Resource-aware [load balancing](@/glossary/load-balancing.md) across monitoring cycles
...
- Alert routing to appropriate handlers (email, webhook, dashboard, [PubSub](@/glossary/pubsub.md))
...
Tracking data feeds directly into [Prismatic HAWKEYE](@/apps/prismatic-hawkeye.md) for visitor correlation -- when a tracked domain's infrastructure changes, HAWKEYE reassesses [risk score](@/glossary/risk-score.md)s for associated traffic. The [KuzuDB graph store](@/apps/prismatic-storage-kuzudb.md) receives entity relationship updates from tracking snapshots, maintaining a temporal graph of entity state evolution. Alerts generated by the tracking system are surfaced in [Prismatic Web](@/apps/prismatic-web.md) dashboards and can trigger automated response workflows through the [agent system](@/apps/prismatic-agents.md).
...
- [Prismatic OSINT Core](@/apps/prismatic-osint-core.md) -- Source adapters for intelligence queries
- [Prismatic Storage Ecto](@/apps/prismatic-storage-ecto.md) -- Snapshot and history persistence
- [Prismatic Storage ETS](@/apps/prismatic-storage-ets.md) -- Active schedule caching
- [Prismatic Telemetry](@/apps/prismatic-telemetry.md) -- Tracking cycle [metrics](@/glossary/metrics.md)

## Related Agents

- [Alert Management Specialist](@/agents/alert-management-specialist.md) -- Manages severity-graded alerts generated by entity change detection
- [Competitor Researcher](@/agents/competitor-researcher.md) -- Leverages tracking infrastructure for competitive entity monitoring
- [Evidence Enforcement Agent](@/agents/evidence-enforcement-agent.md) -- Ensures tracking-based conclusions carry full snapshot provenance

## Related Capabilities

- [Real-Time Monitoring](@/capabilities/real-time-monitoring.md) -- Continuous entity state monitoring with configurable check intervals
- [Intelligence Synthesis](@/capabilities/intelligence-synthesis.md) -- Multi-source tracking data fusion for comprehensive entity profiles
- [Telemetry Integration](@/capabilities/telemetry-integration.md) -- Tracking cycle performance metrics emitted through the telemetry pipeline
