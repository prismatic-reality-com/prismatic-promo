...
- [ETS](@/glossary/ets.md) -- Erlang Term Storage, the primary execution context for match specifications
- [Named Table](@/glossary/named-table.md) -- ETS tables that match specs query against by atom name
- [Ordered Set](@/glossary/ordered-set.md) -- ETS table type supporting range queries with match specs and `select_reverse/2`
- [Process](@/glossary/process.md) -- BEAM processes that own ETS tables and receive match spec results
- [Memory](@/glossary/memory.md) -- memory efficiency achieved through match spec zero-copy filtering
- [Pattern Matching](@/glossary/pattern-matching.md) -- the foundational BEAM capability that match specs formalize
- [Guard](/glossary/guard/) -- guard expressions used in match spec filter conditions
- [Profiling](@/glossary/profiling.md) -- trace-based profiling using match specifications for selective tracing
- [Trace](@/glossary/trace.md) -- the BEAM trace system that uses match specs for filtering
- [GenServer](@/glossary/genserver.md) -- processes that commonly own and query ETS tables via match specs
...
- [Architecture](@/architecture/_index.md) -- ETS and data access architecture in the Prismatic Platform
- [Capabilities](@/capabilities/_index.md) -- high-performance query capabilities powered by match specs
