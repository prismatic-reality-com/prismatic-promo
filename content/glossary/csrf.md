...
CSRF tokens must be generated using a [CSPRNG](@/glossary/csprng.md) to prevent prediction attacks. The token must have sufficient entropy (at least 128 bits) and must be bound to the user's session to prevent token fixation.
...
- [CSPRNG](@/glossary/csprng.md) -- Secure random generation for CSRF tokens
- **XSS** -- Complementary web vulnerability class
- [Session](@/glossary/session.md) -- Session management CSRF relies on
- [OWASP](@/glossary/owasp.md) -- Security standards defining CSRF prevention
