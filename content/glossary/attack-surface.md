...
An attack surface is the total set of points (attack vectors) where an unauthorized user can attempt to enter data into, extract data from, or otherwise interact with a system without authorization. It encompasses all externally reachable infrastructure: exposed network services and open ports, API endpoints, web applications, domain names and subdomains, SSL/[TLS](/glossary/tls/) certificates, DNS records, cloud resources (storage buckets, serverless functions, container registries), email servers, VPN gateways, remote access interfaces, and any other component accessible from outside the organizational perimeter. The attack surface also includes the human dimension -- employees susceptible to phishing, social engineering, or credential theft.
...
The modern attack surface extends far beyond traditional network perimeters. Cloud adoption, SaaS integrations, third-party supply chains, remote work infrastructure, and the proliferation of APIs have expanded the typical organization's attack surface by orders of magnitude. External Attack Surface Management ([EASM](/glossary/easm/)) specifically addresses the externally visible portion of this surface -- what an attacker can discover and target from the public internet, without any insider access or credentials.

## Context in Prismatic

The Prismatic Platform's [EASM](/glossary/easm/) module, part of Prismatic Perimeter, automates attack surface discovery and continuous monitoring for organizations. The scanner probes domains, IP ranges, certificates, DNS records, cloud resources, and third-party integrations to build a comprehensive inventory of an organization's external exposure. Each discovered asset is classified by type, assigned a [risk score](/glossary/risk-score/), monitored for changes, and mapped into the platform's [knowledge graph](/glossary/knowledge-graph/) for relationship analysis. The system competes with commercial tools like BitSight, Black Kite, and SecurityScorecard, providing security ratings (A-F grades with numeric scores 300-900) and compliance assessment against NIS2 and ZKB frameworks.

The platform's own attack surface is minimized through architectural decisions: only two Phoenix [endpoints](/glossary/endpoint/) are exposed (port 4000 for the dashboard, port 4004 for the [REST API](/glossary/rest-api/)), all traffic is [TLS](/glossary/tls/)-encrypted, authentication uses [JWT](/glossary/jwt/) tokens with [RBAC](/glossary/rbac/), and [rate limiting](/glossary/rate-limiting/) prevents abuse. The [Color Teams](/glossary/color-teams/) continuously assess the platform's own security posture through adversarial simulation.
...
|-----------|-----------------|-------------|---------|
| **Open Ports** | Port scanning ([Shodan](/glossary/shodan/)) | Exposed services | SSH on 22, HTTP on 80/443 |
...
| **TLS Certificates** | CT log monitoring ([Censys](/glossary/censys/)) | Domain discovery | *.internal.example.com |
...
| **API Endpoints** | [REST](/glossary/rest-api/)/[GraphQL](/glossary/graphql/) interfaces | Auth bypass, data exposure | [OpenAPI](/glossary/openapi/) validation, RBAC |
| **Dependencies** | Third-party libraries | Supply chain attacks | [Hex](/glossary/hex/) audit, lockfile verification |
| **Containers** | [Docker](/glossary/docker/) images | Base image vulnerabilities | Minimal images, regular updates |
| **Runtime** | [BEAM](/glossary/beam/) VM, ERTS | Runtime exploits | Current Erlang/Elixir versions |
...
| **[Encryption at Rest](/glossary/encryption-at-rest/)** | Encrypt all stored data | Data protection if storage breached |
| **API Hardening** | Schema validation, [rate limiting](/glossary/rate-limiting/), auth | Reduced API abuse surface |
...
|--------|-------------|------------------|----------|
| **[Shodan](/glossary/shodan/)** | Open ports, service banners, vulnerabilities | Continuous scanning | Global IPv4 |
| **[Censys](/glossary/censys/)** | Certificates, hosts, protocols | Weekly full scan | Global internet |
| **[GreyNoise](/glossary/greynoise/)** | Internet noise classification | Real-time | IP intelligence |
...
- [EASM](/glossary/easm/) - External Attack Surface Management system
- [Shodan](/glossary/shodan/) - Internet-wide port and service scanner
- [Censys](/glossary/censys/) - Certificate and host discovery platform
- [GreyNoise](/glossary/greynoise/) - Internet noise and scanner classification
- [Risk Score](/glossary/risk-score/) - Quantified risk assessment per asset
- [Color Teams](/glossary/color-teams/) - Security teams analyzing attack surfaces
- [TLS](/glossary/tls/) - Transport encryption assessed in attack surface analysis
- [REST API](/glossary/rest-api/) - API endpoints constituting software attack surface
- [Rate Limiting](/glossary/rate-limiting/) - Protection reducing API attack surface exposure
- [Encryption at Rest](/glossary/encryption-at-rest/) - Data protection for stored assets
