+++
title = "Live experiment controller"
description = "Live experiment controller — An academic treatment of meta-simulation and cross-disciplinary integration within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Live", "experiment", "controller", "Prismatic-native", "Cross-disciplinary", "Meta-Simulation", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Live experiment controller - Prismatic Platform"
...
- **Formal specification** of live experiment controller requirements using co-simulation frameworks grounded in Tolk (2012) 'Engineering Principles of Combat Modeling and Distributed Simulation'
...
The theoretical underpinnings of live experiment controller draw from multiple disciplines. Tolk (2012) 'Engineering Principles of Combat Modeling and Distributed Simulation' established foundational principles that continue to inform modern approaches. Subsequent work by Vangheluwe (2000) 'DEVS as a Common Denominator for Multi-Formalism Modelling' extended these ideas to computational settings, introducing formal frameworks for multi-fidelity simulation.
...
Despite significant progress, existing solutions for live experiment controller exhibit several shortcomings that motivate our work:
...
The live experiment controller system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule LiveExperimentController.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {LiveExperimentController.Registry, []},
      {LiveExperimentController.Coordinator, []},
      {LiveExperimentController.AgentSupervisor, []},
      {LiveExperimentController.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of live experiment controller:
...
Blackboard.post(:live_experiment_controller, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:live_experiment_controller, fn event ->
...
We formalize live experiment controller as a multi-objective optimization problem over the space of cross-domain transfer. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.LiveExperimentController do
  @moduledoc """
  Live experiment controller — Production implementation.
...
```elixir
defmodule PrismaticWeb.LiveExperimentControllerLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "live-experiment-controller:pipeline")
    end
...
The success of the multi-agent blackboard architecture for live experiment controller validates several key design principles:
...
Deployment of live experiment controller in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :live_experiment_controller, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, LiveExperimentController,
  ...
  telemetry_prefix: [:prismatic, :live_experiment_controller]
```
...
|----------|--------|-------------|
| `/api/v1/live-experiment-controller/analyze` | POST | Submit input for analysis |
| `/api/v1/live-experiment-controller/status/:id` | GET | Check analysis status |
| `/api/v1/live-experiment-controller/result/:id` | GET | Retrieve analysis result |
| `/api/v1/live-experiment-controller/replay/:id` | GET | Replay analysis session |
