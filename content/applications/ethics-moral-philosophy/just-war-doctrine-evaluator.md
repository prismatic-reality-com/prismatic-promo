+++
title = "Just war doctrine evaluator"
description = "Just war doctrine evaluator — An academic treatment of computational ethics and moral philosophy within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["war", "doctrine", "evaluator", "Prismatic-native", "Ethics", "Moral", "Philosophy", "applications", "Prismatic Platform", "Prismatic"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Just war doctrine evaluator - Prismatic Platform"
glossary_terms = ["elixir", "otp", "genserver", "liveview", "ets"]
+++

# Just war doctrine evaluator
...
- **Formal specification** of just war doctrine evaluator requirements using moral graph construction grounded in Rawls (1971) 'A Theory of Justice'
...
Despite significant progress, existing solutions for just war doctrine evaluator exhibit several shortcomings that motivate our work:
...
The just war doctrine evaluator system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule JustWarDoctrineEvaluator.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {JustWarDoctrineEvaluator.Registry, []},
      {JustWarDoctrineEvaluator.Coordinator, []},
      {JustWarDoctrineEvaluator.AgentSupervisor, []},
      {JustWarDoctrineEvaluator.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of just war doctrine evaluator:
...
Blackboard.post(:just_war_doctrine_evaluator, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:just_war_doctrine_evaluator, fn event ->
...
We formalize just war doctrine evaluator as a multi-objective optimization problem over the space of virtue ethics. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.JustWarDoctrineEvaluator do
  @moduledoc """
  Just war doctrine evaluator — Production implementation.
...
```elixir
defmodule PrismaticWeb.JustWarDoctrineEvaluatorLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "just-war-doctrine-evaluator:pipeline")
    end
...
The success of the multi-agent blackboard architecture for just war doctrine evaluator validates several key design principles:
...
Deployment of just war doctrine evaluator in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :just_war_doctrine_evaluator, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, JustWarDoctrineEvaluator,
  ...
  telemetry_prefix: [:prismatic, :just_war_doctrine_evaluator]
```
...
|----------|--------|-------------|
| `/api/v1/just-war-doctrine-evaluator/analyze` | POST | Submit input for analysis |
| `/api/v1/just-war-doctrine-evaluator/status/:id` | GET | Check analysis status |
| `/api/v1/just-war-doctrine-evaluator/result/:id` | GET | Retrieve analysis result |
| `/api/v1/just-war-doctrine-evaluator/replay/:id` | GET | Replay analysis session |
