...
description = "Suicidal ideation detection — An academic treatment of crisis management and negotiation science within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Suicidal", "ideation", "detection", "Prismatic-native", "Crisis", "Intervention", "Negotiation", "applications", "Prismatic Platform", "Prismatic"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Suicidal ideation detection - Prismatic Platform"
...
- **Formal specification** of suicidal ideation detection requirements using motivational interviewing grounded in Fisher & Ury (1981) 'Getting to Yes'
...
Despite significant progress, existing solutions for suicidal ideation detection exhibit several shortcomings that motivate our work:
...
The suicidal ideation detection system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule SuicidalIdeationDetection.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {SuicidalIdeationDetection.Registry, []},
      {SuicidalIdeationDetection.Coordinator, []},
      {SuicidalIdeationDetection.AgentSupervisor, []},
      {SuicidalIdeationDetection.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of suicidal ideation detection:
...
Blackboard.post(:suicidal_ideation_detection, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:suicidal_ideation_detection, fn event ->
...
We formalize suicidal ideation detection as a multi-objective optimization problem over the space of psychological first aid. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.SuicidalIdeationDetection do
  @moduledoc """
  Suicidal ideation detection — Production implementation.
...
```elixir
defmodule PrismaticWeb.SuicidalIdeationDetectionLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "suicidal-ideation-detection:pipeline")
    end
...
The success of the multi-agent blackboard architecture for suicidal ideation detection validates several key design principles:
...
Deployment of suicidal ideation detection in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :suicidal_ideation_detection, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, SuicidalIdeationDetection,
  ...
  telemetry_prefix: [:prismatic, :suicidal_ideation_detection]
```
...
|----------|--------|-------------|
| `/api/v1/suicidal-ideation-detection/analyze` | POST | Submit input for analysis |
| `/api/v1/suicidal-ideation-detection/status/:id` | GET | Check analysis status |
| `/api/v1/suicidal-ideation-detection/result/:id` | GET | Retrieve analysis result |
| `/api/v1/suicidal-ideation-detection/replay/:id` | GET | Replay analysis session |
