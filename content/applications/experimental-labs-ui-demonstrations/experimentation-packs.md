+++
title = "Experimentation packs"
description = "Experimentation packs — An academic treatment of experimental design and user interface research within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Experimentation", "packs", "Prismatic-native", "Experimental", "Labs", "Demonstrations", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Experimentation packs - Prismatic Platform"
glossary_terms = ["elixir", "otp", "genserver", "liveview", "ets"]
+++

# Experimentation packs
...
- **Formal specification** of experimentation packs requirements using randomized controlled trials grounded in Nielsen (1993) 'Usability Engineering'
...
Despite significant progress, existing solutions for experimentation packs exhibit several shortcomings that motivate our work:
...
The experimentation packs system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule ExperimentationPacks.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {ExperimentationPacks.Registry, []},
      {ExperimentationPacks.Coordinator, []},
      {ExperimentationPacks.AgentSupervisor, []},
      {ExperimentationPacks.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of experimentation packs:
...
Blackboard.post(:experimentation_packs, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:experimentation_packs, fn event ->
...
We formalize experimentation packs as a multi-objective optimization problem over the space of information scent. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.ExperimentationPacks do
  @moduledoc """
  Experimentation packs — Production implementation.
...
```elixir
defmodule PrismaticWeb.ExperimentationPacksLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "experimentation-packs:pipeline")
    end
...
The success of the multi-agent blackboard architecture for experimentation packs validates several key design principles:
...
Deployment of experimentation packs in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :experimentation_packs, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, ExperimentationPacks,
  ...
  telemetry_prefix: [:prismatic, :experimentation_packs]
```
...
|----------|--------|-------------|
| `/api/v1/experimentation-packs/analyze` | POST | Submit input for analysis |
| `/api/v1/experimentation-packs/status/:id` | GET | Check analysis status |
| `/api/v1/experimentation-packs/result/:id` | GET | Retrieve analysis result |
| `/api/v1/experimentation-packs/replay/:id` | GET | Replay analysis session |
