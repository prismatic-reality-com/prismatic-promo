+++
title = "Recursive ToM competition"
description = "Recursive ToM competition — An academic treatment of consciousness studies and phenomenological computation within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Recursive", "ToM", "competition", "Prismatic-native", "Consciousness", "Research", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Recursive ToM competition - Prismatic Platform"
glossary_terms = ["elixir", "otp", "genserver", "liveview", "ets"]
+++

# Recursive ToM competition
...
- **Formal specification** of recursive tom competition requirements using information integration measurement (Phi) grounded in Tononi (2004) 'Information Integration Theory of Consciousness'
...
Despite significant progress, existing solutions for recursive tom competition exhibit several shortcomings that motivate our work:
...
The recursive tom competition system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule RecursiveTomCompetition.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {RecursiveTomCompetition.Registry, []},
      {RecursiveTomCompetition.Coordinator, []},
      {RecursiveTomCompetition.AgentSupervisor, []},
      {RecursiveTomCompetition.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of recursive tom competition:
...
Blackboard.post(:recursive_tom_competition, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:recursive_tom_competition, fn event ->
...
We formalize recursive tom competition as a multi-objective optimization problem over the space of attentional processing. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.RecursiveTomCompetition do
  @moduledoc """
  Recursive ToM competition — Production implementation.
...
```elixir
defmodule PrismaticWeb.RecursiveTomCompetitionLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "recursive-tom-competition:pipeline")
    end
...
The success of the multi-agent blackboard architecture for recursive tom competition validates several key design principles:
...
Deployment of recursive tom competition in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :recursive_tom_competition, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, RecursiveTomCompetition,
  ...
  telemetry_prefix: [:prismatic, :recursive_tom_competition]
```
...
|----------|--------|-------------|
| `/api/v1/recursive-tom-competition/analyze` | POST | Submit input for analysis |
| `/api/v1/recursive-tom-competition/status/:id` | GET | Check analysis status |
| `/api/v1/recursive-tom-competition/result/:id` | GET | Retrieve analysis result |
| `/api/v1/recursive-tom-competition/replay/:id` | GET | Replay analysis session |
...
- [Mirror neuron ToM simulation](@/applications/neuroscience-neurobiology/mirror-neuron-tom-simulation.md) — *Neuroscience Neurobiology*
...
- [Comparative ToM studies](@/applications/academic-research-epistemology/comparative-tom-studies.md) — *Academic Research Epistemology*
