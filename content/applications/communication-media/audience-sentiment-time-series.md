+++
title = "Audience sentiment time-series"
description = "Audience sentiment time-series — An academic treatment of communication theory and media analytics within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Audience", "sentiment", "time-series", "Prismatic-native", "Communication", "Media", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Audience sentiment time-series - Prismatic Platform"
glossary_terms = ["elixir", "otp", "genserver", "liveview", "ets"]
+++

# Audience sentiment time-series
...
- **Formal specification** of audience sentiment time-series requirements using natural language processing grounded in McLuhan (1964) 'Understanding Media'
...
Despite significant progress, existing solutions for audience sentiment time-series exhibit several shortcomings that motivate our work:
...
The audience sentiment time-series system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule AudienceSentimentTimeSeries.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {AudienceSentimentTimeSeries.Registry, []},
      {AudienceSentimentTimeSeries.Coordinator, []},
      {AudienceSentimentTimeSeries.AgentSupervisor, []},
      {AudienceSentimentTimeSeries.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of audience sentiment time-series:
...
Blackboard.post(:audience_sentiment_time_series, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:audience_sentiment_time_series, fn event ->
...
We formalize audience sentiment time-series as a multi-objective optimization problem over the space of disinformation detection. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.AudienceSentimentTimeSeries do
  @moduledoc """
  Audience sentiment time-series — Production implementation.
...
```elixir
defmodule PrismaticWeb.AudienceSentimentTimeSeriesLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "audience-sentiment-time-series:pipeline")
    end
...
The success of the multi-agent blackboard architecture for audience sentiment time-series validates several key design principles:
...
Deployment of audience sentiment time-series in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :audience_sentiment_time_series, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, AudienceSentimentTimeSeries,
  ...
  telemetry_prefix: [:prismatic, :audience_sentiment_time_series]
```
...
|----------|--------|-------------|
| `/api/v1/audience-sentiment-time-series/analyze` | POST | Submit input for analysis |
| `/api/v1/audience-sentiment-time-series/status/:id` | GET | Check analysis status |
| `/api/v1/audience-sentiment-time-series/result/:id` | GET | Retrieve analysis result |
| `/api/v1/audience-sentiment-time-series/replay/:id` | GET | Replay analysis session |
