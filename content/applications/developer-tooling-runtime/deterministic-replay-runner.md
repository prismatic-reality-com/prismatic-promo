+++
title = "Deterministic replay runner"
description = "Deterministic replay runner — An academic treatment of developer experience and runtime system engineering within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Deterministic", "replay", "runner", "Prismatic-native", "Developer", "Tooling", "Runtime", "applications", "Prismatic Platform", "Prismatic"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Deterministic replay runner - Prismatic Platform"
...
- **Formal specification** of deterministic replay runner requirements using static analysis grounded in Aho et al. (2006) 'Compilers: Principles, Techniques, and Tools'
...
Despite significant progress, existing solutions for deterministic replay runner exhibit several shortcomings that motivate our work:
...
The deterministic replay runner system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule DeterministicReplayRunner.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {DeterministicReplayRunner.Registry, []},
      {DeterministicReplayRunner.Coordinator, []},
      {DeterministicReplayRunner.AgentSupervisor, []},
      {DeterministicReplayRunner.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of deterministic replay runner:
...
Blackboard.post(:deterministic_replay_runner, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:deterministic_replay_runner, fn event ->
...
We formalize deterministic replay runner as a multi-objective optimization problem over the space of build system optimization. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.DeterministicReplayRunner do
  @moduledoc """
  Deterministic replay runner — Production implementation.
...
```elixir
defmodule PrismaticWeb.DeterministicReplayRunnerLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "deterministic-replay-runner:pipeline")
    end
...
The success of the multi-agent blackboard architecture for deterministic replay runner validates several key design principles:
...
Deployment of deterministic replay runner in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :deterministic_replay_runner, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, DeterministicReplayRunner,
  ...
  telemetry_prefix: [:prismatic, :deterministic_replay_runner]
```
...
|----------|--------|-------------|
| `/api/v1/deterministic-replay-runner/analyze` | POST | Submit input for analysis |
| `/api/v1/deterministic-replay-runner/status/:id` | GET | Check analysis status |
| `/api/v1/deterministic-replay-runner/result/:id` | GET | Retrieve analysis result |
| `/api/v1/deterministic-replay-runner/replay/:id` | GET | Replay analysis session |
