+++
title = "After-action review automation"
description = "After-action review automation — An academic treatment of defense systems and security operations within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["After-action", "review", "automation", "Prismatic-native", "Defense", "Security", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "After-action review automation - Prismatic Platform"
...
- **Formal specification** of after-action review automation requirements using threat modeling (STRIDE, DREAD) grounded in Clausewitz (1832) 'On War'
...
The theoretical underpinnings of after-action review automation draw from multiple disciplines. Clausewitz (1832) 'On War' established foundational principles that continue to inform modern approaches. Subsequent work by Hutchins et al. (2011) 'Intelligence-Driven Computer Network Defense' extended these ideas to computational settings, introducing formal frameworks for red team operations.
...
Despite significant progress, existing solutions for after-action review automation exhibit several shortcomings that motivate our work:
...
The after-action review automation system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule AfterActionReviewAutomation.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {AfterActionReviewAutomation.Registry, []},
      {AfterActionReviewAutomation.Coordinator, []},
      {AfterActionReviewAutomation.AgentSupervisor, []},
      {AfterActionReviewAutomation.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of after-action review automation:
...
Blackboard.post(:after_action_review_automation, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:after_action_review_automation, fn event ->
...
We formalize after-action review automation as a multi-objective optimization problem over the space of command and control. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.AfterActionReviewAutomation do
  @moduledoc """
  After-action review automation — Production implementation.
...
```elixir
defmodule PrismaticWeb.AfterActionReviewAutomationLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "after-action-review-automation:pipeline")
    end
...
The success of the multi-agent blackboard architecture for after-action review automation validates several key design principles:
...
Deployment of after-action review automation in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :after_action_review_automation, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, AfterActionReviewAutomation,
  ...
  telemetry_prefix: [:prismatic, :after_action_review_automation]
```
...
|----------|--------|-------------|
| `/api/v1/after-action-review-automation/analyze` | POST | Submit input for analysis |
| `/api/v1/after-action-review-automation/status/:id` | GET | Check analysis status |
| `/api/v1/after-action-review-automation/result/:id` | GET | Retrieve analysis result |
| `/api/v1/after-action-review-automation/replay/:id` | GET | Replay analysis session |
