...
description = "Insider threat detection — An academic treatment of defense systems and security operations within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Insider", "threat", "detection", "Prismatic-native", "Defense", "Security", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Insider threat detection - Prismatic Platform"
...
- **Formal specification** of insider threat detection requirements using threat modeling (STRIDE, DREAD) grounded in Clausewitz (1832) 'On War'
...
The theoretical underpinnings of insider threat detection draw from multiple disciplines. Clausewitz (1832) 'On War' established foundational principles that continue to inform modern approaches. Subsequent work by Hutchins et al. (2011) 'Intelligence-Driven Computer Network Defense' extended these ideas to computational settings, introducing formal frameworks for red team operations.
...
Despite significant progress, existing solutions for insider threat detection exhibit several shortcomings that motivate our work:
...
The insider threat detection system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule InsiderThreatDetection.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {InsiderThreatDetection.Registry, []},
      {InsiderThreatDetection.Coordinator, []},
      {InsiderThreatDetection.AgentSupervisor, []},
      {InsiderThreatDetection.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of insider threat detection:
...
Blackboard.post(:insider_threat_detection, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:insider_threat_detection, fn event ->
...
We formalize insider threat detection as a multi-objective optimization problem over the space of command and control. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.InsiderThreatDetection do
  @moduledoc """
  Insider threat detection — Production implementation.
...
```elixir
defmodule PrismaticWeb.InsiderThreatDetectionLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "insider-threat-detection:pipeline")
    end
...
The success of the multi-agent blackboard architecture for insider threat detection validates several key design principles:
...
Deployment of insider threat detection in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :insider_threat_detection, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, InsiderThreatDetection,
  ...
  telemetry_prefix: [:prismatic, :insider_threat_detection]
```
...
|----------|--------|-------------|
| `/api/v1/insider-threat-detection/analyze` | POST | Submit input for analysis |
| `/api/v1/insider-threat-detection/status/:id` | GET | Check analysis status |
| `/api/v1/insider-threat-detection/result/:id` | GET | Retrieve analysis result |
| `/api/v1/insider-threat-detection/replay/:id` | GET | Replay analysis session |
...
- [Insider leak early warning](@/applications/intelligence-counter-intelligence/insider-leak-early-warning.md) — *Intelligence Counter Intelligence*
