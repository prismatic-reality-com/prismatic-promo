+++
title = "Populism detection"
description = "Populism detection — An academic treatment of multi-agent systems and artificial societies within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Populism", "detection", "Prismatic-native", "Multi-Agent", "Societies", "applications", "Prismatic Platform", "Prismatic", "Blackboard", "Architecture"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Populism detection - Prismatic Platform"
...
- **Formal specification** of populism detection requirements using agent-based modeling grounded in Wooldridge (2009) 'An Introduction to MultiAgent Systems'
...
Despite significant progress, existing solutions for populism detection exhibit several shortcomings that motivate our work:
...
The populism detection system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule PopulismDetection.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {PopulismDetection.Registry, []},
      {PopulismDetection.Coordinator, []},
      {PopulismDetection.AgentSupervisor, []},
      {PopulismDetection.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of populism detection:
...
Blackboard.post(:populism_detection, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:populism_detection, fn event ->
...
We formalize populism detection as a multi-objective optimization problem over the space of swarm behavior. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.PopulismDetection do
  @moduledoc """
  Populism detection — Production implementation.
...
```elixir
defmodule PrismaticWeb.PopulismDetectionLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "populism-detection:pipeline")
    end
...
The success of the multi-agent blackboard architecture for populism detection validates several key design principles:
...
Deployment of populism detection in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :populism_detection, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, PopulismDetection,
  ...
  telemetry_prefix: [:prismatic, :populism_detection]
```
...
|----------|--------|-------------|
| `/api/v1/populism-detection/analyze` | POST | Submit input for analysis |
| `/api/v1/populism-detection/status/:id` | GET | Check analysis status |
| `/api/v1/populism-detection/result/:id` | GET | Retrieve analysis result |
| `/api/v1/populism-detection/replay/:id` | GET | Replay analysis session |
...
- [Hiring bias detection](@/applications/business-organizational-training/hiring-bias-detection.md) — *Business Organizational Training*
- [Consciousness detection benchmarks](@/applications/consciousness-research/consciousness-detection-benchmarks.md) — *Consciousness Research*
