+++
title = "Narrative laundering detector"
description = "Narrative laundering detector — An academic treatment of intelligence analysis and counter-intelligence operations within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Narrative", "laundering", "detector", "Prismatic-native", "Intelligence", "Counter-intelligence", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Narrative laundering detector - Prismatic Platform"
glossary_terms = ["elixir", "otp", "genserver", "liveview", "ets"]
+++

# Narrative laundering detector
...
- **Formal specification** of narrative laundering detector requirements using ACH (Analysis of Competing Hypotheses) grounded in Heuer (1999) 'Psychology of Intelligence Analysis'
...
The theoretical underpinnings of narrative laundering detector draw from multiple disciplines. Heuer (1999) 'Psychology of Intelligence Analysis' established foundational principles that continue to inform modern approaches. Subsequent work by Clark (2019) 'Intelligence Analysis: A Target-Centric Approach' extended these ideas to computational settings, introducing formal frameworks for collection management.
...
Despite significant progress, existing solutions for narrative laundering detector exhibit several shortcomings that motivate our work:
...
The narrative laundering detector system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule NarrativeLaunderingDetector.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {NarrativeLaunderingDetector.Registry, []},
      {NarrativeLaunderingDetector.Coordinator, []},
      {NarrativeLaunderingDetector.AgentSupervisor, []},
      {NarrativeLaunderingDetector.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of narrative laundering detector:
...
Blackboard.post(:narrative_laundering_detector, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:narrative_laundering_detector, fn event ->
...
We formalize narrative laundering detector as a multi-objective optimization problem over the space of human intelligence. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.NarrativeLaunderingDetector do
  @moduledoc """
  Narrative laundering detector — Production implementation.
...
```elixir
defmodule PrismaticWeb.NarrativeLaunderingDetectorLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "narrative-laundering-detector:pipeline")
    end
...
The success of the multi-agent blackboard architecture for narrative laundering detector validates several key design principles:
...
Deployment of narrative laundering detector in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :narrative_laundering_detector, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, NarrativeLaunderingDetector,
  ...
  telemetry_prefix: [:prismatic, :narrative_laundering_detector]
```
...
|----------|--------|-------------|
| `/api/v1/narrative-laundering-detector/analyze` | POST | Submit input for analysis |
| `/api/v1/narrative-laundering-detector/status/:id` | GET | Check analysis status |
| `/api/v1/narrative-laundering-detector/result/:id` | GET | Retrieve analysis result |
| `/api/v1/narrative-laundering-detector/replay/:id` | GET | Replay analysis session |
...
- [Interview adversarial framing detector](@/applications/communication-media/interview-adversarial-framing-detector.md) — *Communication Media*
- [Narrative framing sandbox](@/applications/communication-media/narrative-framing-sandbox.md) — *Communication Media*
- [Populist narrative detection](@/applications/content-moderation-manipulation/populist-narrative-detection.md) — *Content Moderation Manipulation*
- [Deepfake narrative simulation](@/applications/content-moderation-manipulation/deepfake-narrative-simulation.md) — *Content Moderation Manipulation*
