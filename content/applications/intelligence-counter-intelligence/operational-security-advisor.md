+++
title = "Operational security advisor"
description = "Operational security advisor — An academic treatment of intelligence analysis and counter-intelligence operations within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Operational", "security", "advisor", "Prismatic-native", "Intelligence", "Counter-intelligence", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Operational security advisor - Prismatic Platform"
...
- **Formal specification** of operational security advisor requirements using ACH (Analysis of Competing Hypotheses) grounded in Heuer (1999) 'Psychology of Intelligence Analysis'
...
The theoretical underpinnings of operational security advisor draw from multiple disciplines. Heuer (1999) 'Psychology of Intelligence Analysis' established foundational principles that continue to inform modern approaches. Subsequent work by Clark (2019) 'Intelligence Analysis: A Target-Centric Approach' extended these ideas to computational settings, introducing formal frameworks for collection management.
...
Despite significant progress, existing solutions for operational security advisor exhibit several shortcomings that motivate our work:
...
The operational security advisor system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule OperationalSecurityAdvisor.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {OperationalSecurityAdvisor.Registry, []},
      {OperationalSecurityAdvisor.Coordinator, []},
      {OperationalSecurityAdvisor.AgentSupervisor, []},
      {OperationalSecurityAdvisor.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of operational security advisor:
...
Blackboard.post(:operational_security_advisor, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:operational_security_advisor, fn event ->
...
We formalize operational security advisor as a multi-objective optimization problem over the space of human intelligence. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.OperationalSecurityAdvisor do
  @moduledoc """
  Operational security advisor — Production implementation.
...
```elixir
defmodule PrismaticWeb.OperationalSecurityAdvisorLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "operational-security-advisor:pipeline")
    end
...
The success of the multi-agent blackboard architecture for operational security advisor validates several key design principles:
...
Deployment of operational security advisor in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :operational_security_advisor, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, OperationalSecurityAdvisor,
  ...
  telemetry_prefix: [:prismatic, :operational_security_advisor]
```
...
|----------|--------|-------------|
| `/api/v1/operational-security-advisor/analyze` | POST | Submit input for analysis |
| `/api/v1/operational-security-advisor/status/:id` | GET | Check analysis status |
| `/api/v1/operational-security-advisor/result/:id` | GET | Retrieve analysis result |
| `/api/v1/operational-security-advisor/replay/:id` | GET | Replay analysis session |
...
- [Crowd security de-escalation](@/applications/defense-security/crowd-security-de-escalation.md) — *Defense Security*
