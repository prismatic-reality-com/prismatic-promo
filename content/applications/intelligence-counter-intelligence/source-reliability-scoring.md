...
description = "Source reliability scoring — An academic treatment of intelligence analysis and counter-intelligence operations within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Source", "reliability", "scoring", "Prismatic-native", "Intelligence", "Counter-intelligence", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Source reliability scoring - Prismatic Platform"
...
- **Formal specification** of source reliability scoring requirements using ACH (Analysis of Competing Hypotheses) grounded in Heuer (1999) 'Psychology of Intelligence Analysis'
...
The theoretical underpinnings of source reliability scoring draw from multiple disciplines. Heuer (1999) 'Psychology of Intelligence Analysis' established foundational principles that continue to inform modern approaches. Subsequent work by Clark (2019) 'Intelligence Analysis: A Target-Centric Approach' extended these ideas to computational settings, introducing formal frameworks for collection management.
...
Despite significant progress, existing solutions for source reliability scoring exhibit several shortcomings that motivate our work:
...
The source reliability scoring system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule SourceReliabilityScoring.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {SourceReliabilityScoring.Registry, []},
      {SourceReliabilityScoring.Coordinator, []},
      {SourceReliabilityScoring.AgentSupervisor, []},
      {SourceReliabilityScoring.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of source reliability scoring:
...
Blackboard.post(:source_reliability_scoring, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:source_reliability_scoring, fn event ->
...
We formalize source reliability scoring as a multi-objective optimization problem over the space of human intelligence. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.SourceReliabilityScoring do
  @moduledoc """
  Source reliability scoring — Production implementation.
...
```elixir
defmodule PrismaticWeb.SourceReliabilityScoringLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "source-reliability-scoring:pipeline")
    end
...
The success of the multi-agent blackboard architecture for source reliability scoring validates several key design principles:
...
Deployment of source reliability scoring in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :source_reliability_scoring, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, SourceReliabilityScoring,
  ...
  telemetry_prefix: [:prismatic, :source_reliability_scoring]
```
...
|----------|--------|-------------|
| `/api/v1/source-reliability-scoring/analyze` | POST | Submit input for analysis |
| `/api/v1/source-reliability-scoring/status/:id` | GET | Check analysis status |
| `/api/v1/source-reliability-scoring/result/:id` | GET | Retrieve analysis result |
| `/api/v1/source-reliability-scoring/replay/:id` | GET | Replay analysis session |
...
- [Editorial headline risk scoring](@/applications/communication-media/editorial-headline-risk-scoring.md) — *Communication Media*
