+++
title = "Populist narrative detection"
description = "Populist narrative detection — An academic treatment of content safety and manipulation detection systems within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Populist", "narrative", "detection", "Prismatic-native", "Content", "Moderation", "Manipulation", "applications", "Prismatic Platform", "Prismatic"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Populist narrative detection - Prismatic Platform"
glossary_terms = ["elixir", "otp", "genserver", "liveview", "ets"]
+++

# Populist narrative detection
...
- **Formal specification** of populist narrative detection requirements using multi-modal classification grounded in Gillespie (2018) 'Custodians of the Internet'
...
The theoretical underpinnings of populist narrative detection draw from multiple disciplines. Gillespie (2018) 'Custodians of the Internet' established foundational principles that continue to inform modern approaches. Subsequent work by Benkler et al. (2018) 'Network Propaganda' extended these ideas to computational settings, introducing formal frameworks for platform integrity.
...
Despite significant progress, existing solutions for populist narrative detection exhibit several shortcomings that motivate our work:
...
The populist narrative detection system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule PopulistNarrativeDetection.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {PopulistNarrativeDetection.Registry, []},
      {PopulistNarrativeDetection.Coordinator, []},
      {PopulistNarrativeDetection.AgentSupervisor, []},
      {PopulistNarrativeDetection.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of populist narrative detection:
...
Blackboard.post(:populist_narrative_detection, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:populist_narrative_detection, fn event ->
...
We formalize populist narrative detection as a multi-objective optimization problem over the space of hate speech classifiers. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.PopulistNarrativeDetection do
  @moduledoc """
  Populist narrative detection — Production implementation.
...
```elixir
defmodule PrismaticWeb.PopulistNarrativeDetectionLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "populist-narrative-detection:pipeline")
    end
...
The success of the multi-agent blackboard architecture for populist narrative detection validates several key design principles:
...
Deployment of populist narrative detection in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :populist_narrative_detection, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, PopulistNarrativeDetection,
  ...
  telemetry_prefix: [:prismatic, :populist_narrative_detection]
```
...
|----------|--------|-------------|
| `/api/v1/populist-narrative-detection/analyze` | POST | Submit input for analysis |
| `/api/v1/populist-narrative-detection/status/:id` | GET | Check analysis status |
| `/api/v1/populist-narrative-detection/result/:id` | GET | Retrieve analysis result |
| `/api/v1/populist-narrative-detection/replay/:id` | GET | Replay analysis session |
...
- [Narrative laundering detector](@/applications/intelligence-counter-intelligence/narrative-laundering-detector.md) — *Intelligence Counter Intelligence*
