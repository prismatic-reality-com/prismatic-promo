+++
title = "Rhetorical device library detection"
description = "Rhetorical device library detection — An academic treatment of content safety and manipulation detection systems within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Rhetorical", "device", "library", "detection", "Prismatic-native", "Content", "Moderation", "Manipulation", "applications", "Prismatic Platform"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Rhetorical device library detection - Prismatic Platform"
glossary_terms = ["elixir", "otp", "genserver", "liveview", "ets"]
+++

# Rhetorical device library detection
...
- **Formal specification** of rhetorical device library detection requirements using multi-modal classification grounded in Gillespie (2018) 'Custodians of the Internet'
...
The theoretical underpinnings of rhetorical device library detection draw from multiple disciplines. Gillespie (2018) 'Custodians of the Internet' established foundational principles that continue to inform modern approaches. Subsequent work by Benkler et al. (2018) 'Network Propaganda' extended these ideas to computational settings, introducing formal frameworks for platform integrity.
...
Despite significant progress, existing solutions for rhetorical device library detection exhibit several shortcomings that motivate our work:
...
The rhetorical device library detection system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule RhetoricalDeviceLibraryDetection.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {RhetoricalDeviceLibraryDetection.Registry, []},
      {RhetoricalDeviceLibraryDetection.Coordinator, []},
      {RhetoricalDeviceLibraryDetection.AgentSupervisor, []},
      {RhetoricalDeviceLibraryDetection.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of rhetorical device library detection:
...
Blackboard.post(:rhetorical_device_library_detection, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:rhetorical_device_library_detection, fn event ->
...
We formalize rhetorical device library detection as a multi-objective optimization problem over the space of hate speech classifiers. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.RhetoricalDeviceLibraryDetection do
  @moduledoc """
  Rhetorical device library detection — Production implementation.
...
```elixir
defmodule PrismaticWeb.RhetoricalDeviceLibraryDetectionLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "rhetorical-device-library-detection:pipeline")
    end
...
The success of the multi-agent blackboard architecture for rhetorical device library detection validates several key design principles:
...
Deployment of rhetorical device library detection in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :rhetorical_device_library_detection, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, RhetoricalDeviceLibraryDetection,
  ...
  telemetry_prefix: [:prismatic, :rhetorical_device_library_detection]
```
...
|----------|--------|-------------|
| `/api/v1/rhetorical-device-library-detection/analyze` | POST | Submit input for analysis |
| `/api/v1/rhetorical-device-library-detection/status/:id` | GET | Check analysis status |
| `/api/v1/rhetorical-device-library-detection/result/:id` | GET | Retrieve analysis result |
| `/api/v1/rhetorical-device-library-detection/replay/:id` | GET | Replay analysis session |
