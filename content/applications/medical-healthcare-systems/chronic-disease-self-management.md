...
description = "Chronic disease self-management — An academic treatment of health informatics and clinical decision support within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Chronic", "disease", "self-management", "Prismatic-native", "Medical", "Healthcare", "Systems", "applications", "Prismatic Platform", "Prismatic"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Chronic disease self-management - Prismatic Platform"
...
- **Formal specification** of chronic disease self-management requirements using clinical trial design grounded in Sackett et al. (1996) 'Evidence-Based Medicine'
...
Despite significant progress, existing solutions for chronic disease self-management exhibit several shortcomings that motivate our work:
...
The chronic disease self-management system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule ChronicDiseaseSelfManagement.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {ChronicDiseaseSelfManagement.Registry, []},
      {ChronicDiseaseSelfManagement.Coordinator, []},
      {ChronicDiseaseSelfManagement.AgentSupervisor, []},
      {ChronicDiseaseSelfManagement.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of chronic disease self-management:
...
Blackboard.post(:chronic_disease_self_management, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:chronic_disease_self_management, fn event ->
...
We formalize chronic disease self-management as a multi-objective optimization problem over the space of precision medicine. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.ChronicDiseaseSelfManagement do
  @moduledoc """
  Chronic disease self-management — Production implementation.
...
```elixir
defmodule PrismaticWeb.ChronicDiseaseSelfManagementLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "chronic-disease-self-management:pipeline")
    end
...
The success of the multi-agent blackboard architecture for chronic disease self-management validates several key design principles:
...
Deployment of chronic disease self-management in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :chronic_disease_self_management, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, ChronicDiseaseSelfManagement,
  ...
  telemetry_prefix: [:prismatic, :chronic_disease_self_management]
```
...
|----------|--------|-------------|
| `/api/v1/chronic-disease-self-management/analyze` | POST | Submit input for analysis |
| `/api/v1/chronic-disease-self-management/status/:id` | GET | Check analysis status |
| `/api/v1/chronic-disease-self-management/result/:id` | GET | Retrieve analysis result |
| `/api/v1/chronic-disease-self-management/replay/:id` | GET | Replay analysis session |
