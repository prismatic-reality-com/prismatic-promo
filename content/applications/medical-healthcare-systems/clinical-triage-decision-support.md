+++
title = "Clinical triage decision support"
description = "Clinical triage decision support — An academic treatment of health informatics and clinical decision support within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Clinical", "triage", "decision", "support", "Prismatic-native", "Medical", "Healthcare", "Systems", "applications", "Prismatic Platform"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Clinical triage decision support - Prismatic Platform"
...
- **Formal specification** of clinical triage decision support requirements using clinical trial design grounded in Sackett et al. (1996) 'Evidence-Based Medicine'
...
Despite significant progress, existing solutions for clinical triage decision support exhibit several shortcomings that motivate our work:
...
The clinical triage decision support system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule ClinicalTriageDecisionSupport.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {ClinicalTriageDecisionSupport.Registry, []},
      {ClinicalTriageDecisionSupport.Coordinator, []},
      {ClinicalTriageDecisionSupport.AgentSupervisor, []},
      {ClinicalTriageDecisionSupport.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of clinical triage decision support:
...
Blackboard.post(:clinical_triage_decision_support, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:clinical_triage_decision_support, fn event ->
...
We formalize clinical triage decision support as a multi-objective optimization problem over the space of precision medicine. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.ClinicalTriageDecisionSupport do
  @moduledoc """
  Clinical triage decision support — Production implementation.
...
```elixir
defmodule PrismaticWeb.ClinicalTriageDecisionSupportLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "clinical-triage-decision-support:pipeline")
    end
...
The success of the multi-agent blackboard architecture for clinical triage decision support validates several key design principles:
...
Deployment of clinical triage decision support in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :clinical_triage_decision_support, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, ClinicalTriageDecisionSupport,
  ...
  telemetry_prefix: [:prismatic, :clinical_triage_decision_support]
```
...
|----------|--------|-------------|
| `/api/v1/clinical-triage-decision-support/analyze` | POST | Submit input for analysis |
| `/api/v1/clinical-triage-decision-support/status/:id` | GET | Check analysis status |
| `/api/v1/clinical-triage-decision-support/result/:id` | GET | Retrieve analysis result |
| `/api/v1/clinical-triage-decision-support/replay/:id` | GET | Replay analysis session |
