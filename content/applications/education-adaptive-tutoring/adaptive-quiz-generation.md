...
description = "Adaptive quiz generation — An academic treatment of adaptive learning systems and intelligent tutoring within the Prismatic Platform ecosystem, covering theoretical foundations, system architecture, implementation methodology, and empirical evaluation."
date = 2025-09-08
weight = 10
...
keywords = ["Adaptive", "quiz", "generation", "Prismatic-native", "Education", "Tutoring", "applications", "Prismatic Platform", "Prismatic", "Blackboard"]
quality_score = 92
see_also = ["apps", "technologies", "agents", "glossary"]
image = "/images/sections/applications.png"
image_alt = "Adaptive quiz generation - Prismatic Platform"
...
- **Formal specification** of adaptive quiz generation requirements using knowledge space theory grounded in Vygotsky (1978) 'Mind in Society'
...
Despite significant progress, existing solutions for adaptive quiz generation exhibit several shortcomings that motivate our work:
...
The adaptive quiz generation system is implemented as a Prismatic-native application following the platform's standard architectural patterns. The architecture comprises four principal layers:
...
```elixir
defmodule AdaptiveQuizGeneration.Supervisor do
  use Supervisor
  ...
  def init(_opts) do
    children = [
      {AdaptiveQuizGeneration.Registry, []},
      {AdaptiveQuizGeneration.Coordinator, []},
      {AdaptiveQuizGeneration.AgentSupervisor, []},
      {AdaptiveQuizGeneration.ReplayStore, []}
    ]
...
The system employs a heterogeneous agent ensemble where each agent specializes in a distinct aspect of adaptive quiz generation:
...
Blackboard.post(:adaptive_quiz_generation, %{
  agent: :domain_scorer,
  finding: result,
  confidence: 0.87,
...
Blackboard.subscribe(:adaptive_quiz_generation, fn event ->
...
We formalize adaptive quiz generation as a multi-objective optimization problem over the space of learning analytics. Let $\\mathcal{D}$ represent the input domain and $\\mathcal{Y}$ the output space. The system seeks to find mapping $f: \\mathcal{D} \\rightarrow \\mathcal{Y}$ that simultaneously optimizes:
...
```elixir
defmodule Prismatic.Applications.AdaptiveQuizGeneration do
  @moduledoc """
  Adaptive quiz generation — Production implementation.
...
```elixir
defmodule PrismaticWeb.AdaptiveQuizGenerationLive do
  use PrismaticWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Prismatic.PubSub, "adaptive-quiz-generation:pipeline")
    end
...
The success of the multi-agent blackboard architecture for adaptive quiz generation validates several key design principles:
...
Deployment of adaptive quiz generation in production environments requires attention to:
...
- **Monitoring**: Telemetry integration via `[:prismatic, :adaptive_quiz_generation, :*]` events provides comprehensive observability
...
```elixir
# config/config.exs
config :prismatic, AdaptiveQuizGeneration,
  ...
  telemetry_prefix: [:prismatic, :adaptive_quiz_generation]
```
...
|----------|--------|-------------|
| `/api/v1/adaptive-quiz-generation/analyze` | POST | Submit input for analysis |
| `/api/v1/adaptive-quiz-generation/status/:id` | GET | Check analysis status |
| `/api/v1/adaptive-quiz-generation/result/:id` | GET | Retrieve analysis result |
| `/api/v1/adaptive-quiz-generation/replay/:id` | GET | Replay analysis session |
...
- [Replay corpus generation](@/applications/academic-research-epistemology/replay-corpus-generation.md) — *Academic Research Epistemology*
