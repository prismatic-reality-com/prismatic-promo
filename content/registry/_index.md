...
Unified searchable catalog of every [autonomous agent](@/glossary/autonomous-agent.md) and [slash command](@/glossary/slash-command.md) in the Prismatic Platform. 434 [agents](@/glossary/agent.md) and 216 [commands](@/glossary/command.md) span 14 operational [domains](@/glossary/domain.md) -- from [Quality Assurance](@/glossary/quality-assurance.md) and [OSINT Intelligence](@/glossary/osint.md) to [Security Operations](@/glossary/security-operations.md) and Strategic Command -- each registered with [machine-readable](@/glossary/machine-readable.md) [metadata](@/glossary/metadata-management.md) that enables auto-discovery, capability matching, and runtime skill selection.

## Abstract

The Prismatic Platform [Registry](@/glossary/registry.md) is the authoritative inventory of all AI-Assisted Development ([AIAD](@/glossary/aiad.md)) components available within the platform [ecosystem](@/glossary/ecosystem.md). It indexes 434 [autonomous agents](@/glossary/autonomous-agent.md), 216 [slash commands](@/glossary/slash-command.md), and their associated [metadata](@/glossary/metadata-management.md) -- capability declarations, [authority levels](@/glossary/authority-level.md), [domain](@/glossary/domain.md) classifications, tool requirements, and [enforcement policies](@/glossary/enforcement-policy.md) -- into a single searchable catalog. The registry serves three distinct roles: it is the **discovery mechanism** through which operators and orchestration systems locate the right agent for a given task, the **contract surface** that defines what each component can do and under what constraints, and the **quality enforcement checkpoint** that ensures every registered component meets the platform's zero-tolerance compliance requirements before it can be deployed.
...
A platform with hundreds of specialized [agents](@/glossary/agent.md) faces a fundamental discoverability challenge. When an operator needs to fix a [compilation](@/glossary/compilation.md) warning, investigate a domain's [WHOIS](@/glossary/whois.md) history, or orchestrate a multi-phase [CI/CD](@/glossary/ci-cd.md) recovery, they should not need to know the name of the [agent](@/glossary/agent.md) or [command](@/glossary/command.md) that handles that task. The [registry](@/glossary/registry.md) solves this by maintaining a searchable index of capabilities, so queries like "find all agents that can perform [static analysis](@/glossary/static-analysis.md)" or "which command handles genetic evolution" resolve to concrete, deployable components.
...
The [registry](@/glossary/registry.md) is built on the [AIAD](@/glossary/aiad.md) (AI-Assisted Development) standard, a vendor-neutral [specification](@/glossary/specification.md) for describing [AI agents](@/glossary/ai-agent.md), [commands](@/glossary/command.md), [workflows](@/glossary/workflow.md), and [policies](@/glossary/policy.md). [AIAD](@/glossary/aiad.md) provides the [schema](@/glossary/schema.md) language -- `agent-spec` for [agents](@/glossary/agent.md), `command-spec` for [commands](@/glossary/command.md) -- that gives each registry entry a consistent, [machine-readable](@/glossary/machine-readable.md) structure. This consistency is what makes cross-platform support possible: the same [agent](@/glossary/agent.md) definitions work with [Claude Code](@/glossary/claude-code.md), ChatGPT, Gemini, and other [LLM](@/glossary/llm.md) systems because [AIAD](@/glossary/aiad.md) does not depend on any single provider's [API](@/glossary/api.md) or prompt format.
...
At application startup, the `prismatic_agents` [OTP application](@/glossary/otp-application.md) loads [agent](@/glossary/agent.md) definitions from the [registry](@/glossary/registry.md) into an [ETS table](@/glossary/ets.md), making them available for [O(1) lookup](@/glossary/constant-time.md) by [agent](@/glossary/agent.md) ID, [domain](@/glossary/domain.md), capability, or [authority level](@/glossary/authority-level.md). The runtime layer also registers dynamically created [agents](@/glossary/agent.md) -- those defined programmatically rather than through `.agent.md` files -- bringing the total count above the static file count.
...
The four [hierarchy levels](@/glossary/agent-tier.md) establish a clear [chain of command](@/glossary/chain-of-command.md):

**L1 -- [Strategic Supreme](@/glossary/strategic-supreme.md)** (5 [agents](@/glossary/agent.md)). These are the apex coordinators with unlimited tactical [authority](@/glossary/authority-level.md). They [orchestrate](@/glossary/orchestration.md) multi-[agent](@/glossary/agent.md) campaigns, resolve conflicts between lower-level [agents](@/glossary/agent.md), and make platform-wide decisions. Examples include `archer-supreme` ([crisis intervention](@/glossary/crisis-intervention.md) and impossible missions), `supreme-coordinator` (cross-domain [orchestration](@/glossary/orchestration.md)), and `prismatic-supreme-commander` (ultimate platform [authority](@/glossary/authority-level.md)).
...
The 14 operational [domains](@/glossary/domain.md) provide the primary organizational [taxonomy](@/glossary/taxonomy.md) for the [registry](@/glossary/registry.md). Each [domain](@/glossary/domain.md) represents a distinct area of platform capability:
